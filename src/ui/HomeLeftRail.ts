/**
 * 首页左侧可扩展玩法栏（对齐 home_hub_ui_prototype_v4）
 *
 * 与底栏「主线」分工：
 * - 主线 = 章节关卡地图（本页中心）
...
function makeRailButton(item: HomeRailItem): PIXI.Container {
  ...
  const disc = makePanel({
    width: BTN, height: BTN, radius: 18,
  ...
  const tex = item.iconPath ? TextureCache.get(item.iconPath) : null;
  if (tex) {
    const icon = new PIXI.Sprite(tex);
    icon.anchor.set(0.5);
    icon.scale.set(ICON / Math.max(tex.width, tex.height));
