...
export function resolveSystemFontFamily(): string {
  if (Platform.isCanvasHostGuarded) return 'sans-serif';
  return FONT_FAMILY;
}

/** 战斗数字 / 飘字：微信可走紧缩西文，Tap / 华为只用已加载正文字体或系统体 */
export function resolveLatinHudFontFamily(): string {
  const sys = resolveSystemFontFamily();
  const body = getBodyFontFamily();
  if (Platform.isCanvasHostGuarded) return body ? `${body}, ${sys}` : sys;
...
export function resolveCalligraphyFontFamily(): string {
  ...
  const fallback = Platform.isCanvasHostGuarded ? resolveSystemFontFamily() : FONT_FAMILY_DISPLAY;
