...
describe('阶段八新技能整合（实战落地）', () => {
  ...
  const NEW_TEAM = ['pet_007', 'pet_005', 'pet_001', 'pet_002', 'pet_003'];

  it('multiHit：多段伤害累计扣减敌人 HP', () => {
    const ctrl = makeCtrl(NEW_TEAM);
    const idx = ctrl.team.findIndex((p) => p.def.id === 'pet_002');
    ctrl.team[idx].skillCdLeft = 0;
  ...
  it('dot：施加灼烧，敌人回合结束持续掉血', () => {
    const ctrl = makeCtrl(NEW_TEAM);
    const idx = ctrl.team.findIndex((p) => p.def.id === 'pet_007');
    ctrl.team[idx].skillCdLeft = 0;
    ctrl.castSkill(idx);
  ...
  it('stun：眩晕使敌人本回合跳过攻击', () => {
    const ctrl = makeCtrl(NEW_TEAM);
    const idx = ctrl.team.findIndex((p) => p.def.id === 'pet_005');
    // 让敌人本回合本应普攻
  ...
  it('defenseBreak：破防后同一攻击伤害更高', () => {
    ...
    const bIdx = broken.team.findIndex((p) => p.def.id === 'pet_001');
    broken.team[bIdx].skillCdLeft = 0;
    broken.castSkill(bIdx); // 施加破防

    // 同一只多段宠在两个控制器中打出的伤害对比
    const mIdxA = base.team.findIndex((p) => p.def.id === 'pet_002');
    const mIdxB = broken.team.findIndex((p) => p.def.id === 'pet_002');
...
describe('技能效果', () => {
  ...
  it('healPct：按队伍最大生命百分比回血', () => {
    ...
    const ctrl = makeCtrl(['pet_010', 'pet_007']);
  ...
  it('shield：敌人伤害先被护盾吸收', () => {
    ...
    const ctrl = makeCtrl(['pet_019', 'pet_007']);
  ...
  it('dmgBoost：增伤 buff 生效并随敌人回合衰减', () => {
    ...
    const boostTeam = ['pet_025', 'pet_007'];
  ...
  it('convertOrbs：只返回转珠请求，不直接改盘面', () => {
    ...
    const ctrl = makeCtrl(['pet_029', 'pet_007']);
...
describe('阶段十二·角色专属战斗属性（全队聚合）', () => {
  ...
  it('resolveTurn 心珠回血按 teamHealBonus 放大（与 calcHeal 口径一致）', () => {
    const ctrl = makeCtrl(['pet_004', 'pet_007']);
    ctrl.beginResolve();
...
describe('有效珠机制（队伍属性覆盖）', () => {
  it('队伍无该属性时，消除组不产生攻击但仍计 Combo', () => {
    // 纯火队
    const ctrl = makeCtrl(['pet_007', 'pet_008']);
    ctrl.beginResolve();
