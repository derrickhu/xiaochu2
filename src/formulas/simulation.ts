...
export function simulateBattle(
  ...
  for (let turn = 1; turn <= TURN_CAP; turn++) {
    ...
    decayStatuses();
    /*
     * 这里**故意不**镜像 powerBudget.NOVICE_MERCY 的两条行为层规则
     * （灵宠护体一次、空拖不惩罚），别当成漏建模来「补上」：
     *
     * - 空拖不惩罚本来就无从建模：模拟器每回合按 model.combo 稳定拿到消除，
     *   它从不空拖，那条规则救的正是模型没犯的错。
     * - 灵宠护体是刻意不算进难度预算的。一旦模型也能白嫖一次复活，
     *   这一次容错就变成了可以拿去堆数值的额度：以后有人把杂兵血量调回去，
     *   护栏照样跑绿，而真实玩家的容错已经被悄悄花掉了。
     *
     * 结论：难度契约量的是「不靠任何怜悯也过得去」，护体与空拖豁免是纯余量。
     * 所以模拟器算出来的通关难度必然略严于线上，这个方向是安全的。
     */
    if (heroHp <= 0) return finish(false);
  }
