...
describe('1-5 Boss：平滑养成（低手可过不三星，中手节奏适中）', () => {
  ...
  const sim15 = (model = COMBO_MODELS.mid) =>
    simulateBattle(buildTeam(TEAMS.default, BOSS_1_5_LV, S), 'stage_1_8', model);
  ...
  it('未练级（Lv.1，被动全锁）低手打 1-5 会被拦截：解锁被动前有真实养成需求', () => {
    const r = sim(TEAMS.default, 'stage_1_8', COMBO_MODELS.low);
  ...
  it('1-5 多波 Boss：爆发队高手可冲三星', () => {
    expect(sim(TEAMS.burst, 'stage_1_8', COMBO_MODELS.high).stars).toBe(3);
  });
});
...
describe('功率预算：欠养成会卡在新章（不能跳章碾压）', () => {
  it('停留在第 1 章预算（L1/1★）的默认队打不穿第 3 章 Boss', () => {
    ...
    expect(simulateBattle(under, 'stage_3_8', COMBO_MODELS.mid).win).toBe(false);
  });
  ...
  it('只到第 2 章预算（L12/2★）的主队低手打不穿第 3 章 Boss，需继续养成', () => {
    ...
    expect(simulateBattle(ch2Budget, 'stage_3_8', COMBO_MODELS.low).win).toBe(false);
  });

  it('初始 L1/1★ 默认队无法全清第 3 章（终章 Boss 卡关）', () => {
    expect(sim(TEAMS.default, 'stage_3_8', COMBO_MODELS.mid).win).toBe(false);
  });
});
